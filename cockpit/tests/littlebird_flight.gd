extends Node
## Headless: THE MH-6M LITTLE BIRD FLIES, through the pilot's own controls. Read RESULT=.
##
##   Godot --headless --path cockpit res://tests/littlebird_flight.tscn
##
## THE USER'S SECOND STANDARD (2026-09-17): a craft must fly well, and for this one team-lead named the manoeuvres --
## a hover, forward flight, a banked turn and a landing. So a pilot is seated in kind 28 and flies them through the
## frame the wire carries from a seated player's rig (`set_pilot_input`: the collective on the lever, the cyclic as
## pitch and roll, the pedals as rudder), with the same steady proportional robot and the same gains as
## `tests/handling.gd`, which is a REPORT over every craft. This is the Little Bird's GATE: each manoeuvre has a floor.
##
## THE FLOORS, and what they are reasoned from:
##   LIFT       off the ground on the collective in under 3 s, and a best climb of at least 6 m/s -- a light
##              helicopter at 1,406 kg with a 425 shp engine climbs about 10 m/s (the MD 530F's published 2,070 ft/min).
##   HOVER      hands off the cyclic and the pedals for 5 s at the hover: attitude wanders under 8 degrees and the
##              craft drifts under 8 m. A Little Bird is twitchy, not unstable.
##   SPEED      15 degrees nose down for 45 s, height held on the collective: between 55 and 85 m/s. [W] gives a 135 kt
##              (69 m/s) cruise and a 152 kt (78 m/s) top speed.
##   TURN       a 30-degree bank at speed with the feet still: the flight path must turn at at least 85 per cent of a
##              coordinated turn's g tan(bank) / v at the speed it is flying. Until `lane/rotors`' helicopter turn merged (8c4ca685) no
##              helicopter here turned on its bank alone -- this one managed 0.6 deg/s -- so the first version of this
##              suite gated the turn WITH pedal and printed this one; it gates this one now, and prints the pedal's.
##   LANDING    slowed to a hover, let down at 0.8 m/s for the last 15 m: touches under 2 m/s, upright, on its skids.
##
...
const SPEED_BETWEEN: Vector2 = Vector2(55.0, 85.0)
...
func _check(label: String, ok: bool, detail: String) -> void:
	print("[littlebird_flight] %s %s (%s)" % ["PASS" if ok else "FAIL", label, detail])
	if not ok:
...
func _ready() -> void:
	...
	_check("a_pilot_is_seated_in_the_little_bird", seated and int(Sim.geometry_of(kind).get("model", -1)) == Sim.Model.HELICOPTER,
	...
	_check("it_flies_forward_at_a_little_birds_speed", fastest >= SPEED_BETWEEN.x and fastest <= SPEED_BETWEEN.y,
	...
	_check("it_lands_on_its_skids", bool(land["ok"]), String(land["said"]))
	_end()
	_finish()
...
func _land(hy: float) -> Dictionary:
	...
	return {"ok": ok, "said": "touched %s at %.2f m/s down (under %.1f) and %.1f m/s across; at rest %.2f m off its skids' height, %s"
