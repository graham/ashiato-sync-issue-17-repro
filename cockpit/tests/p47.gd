extends Node
## Headless contract for the P-47D-30 airframe: its measured envelope against the published and the printed one, the
## features that make it a Thunderbolt, one object, and the things that MOVE -- the gear's inner doors opening before a leg
## moves and shutting after, the mains folding INWARD and SHORTENING as they go, the tail wheel forward into the fuselage,
## every surface on the stick, the pedals and the flap lever, and the propeller turning clockwise seen from behind -- each
## read back from DRAWN VERTICES, never from a node's transform. Read RESULT=.
##
## THE ENVELOPE AS CONSTANTS AT THE TOP, TYPED, so the reference is visible in one place and no check asks the builder's
## own constant what the builder did. Every MEASURED figure is printed from AN 01-65BC-2's and NACA RM L8A06's three-views
## by `craft/p47/measure_views.py`; the published ones are Wikipedia's for the P-47D; the PRINTED ones are on [AN]'s sheet.
const PUBLISHED_LENGTH := 11.02
const PUBLISHED_SPAN := 12.43
const PUBLISHED_HEIGHT := 4.47
const PRINTED_LENGTH := 11.003     # 36 ft 1-3/16 in, [AN]'s note for the C-1 and later
const PRINTED_SPAN := 12.429       # 40 ft 9-5/16 in
const PRINTED_TRACK := 4.750       # 15 ft 7 in
const PRINTED_PROP := 3.962        # [NACA] 13 ft
const PRINTED_TAILPLANE := 4.879   # 16 ft 0-3/32 in
const PRINTED_GROUND_ANGLE := 12.0
const PRINTED_FIN_OVER_GROUND := 4.171   # 13 ft 8-3/16 in, level on the mains
const DIHEDRAL := 5.56             # degrees, the front view's middles 2.5 to 5.5 m out (6 printed)
const DRAWN_WING_AREA := 27.20     # m2, the drawn planform (26.94) at the printed span; the published 27.87
const BELLY := -1.120              # m under the thrust line under the wing, the turbocharger's ducting
const FIN_TOP := 2.073
const GUN_OUTS: Array = [2.706, 2.867, 3.019, 3.231]   # the front view's four muzzles a wing, at the printed span
const TELESCOPE := 0.229           # 9 in, the mains' shortening
const ELEVATOR_UP := 30.0
const ELEVATOR_DOWN := 20.0
const FLAP_TRAVEL := 40.0
const AILERON_TRAVEL := 15.0

## Every part a reader would name when looking at a P-47D.
const PARTS: Array = ["Fuselage", "Coaming", "Spinner", "Propeller", "Intercooler", "TurboHood", "Wells", "WingStarboard",
	"WingPort", "GunsStarboard", "GunsPort", "AileronStarboard", "AileronPort", "FlapStarboard", "FlapPort",
	"ElevatorStarboard", "ElevatorPort", "Tailplane", "Fin", "Rudder", "MainGearStarboard", "MainGearPort",
	"MainGearStarboardWheel", "MainGearPortWheel", "InnerDoorStarboard", "InnerDoorPort", "TailGear", "TailDoorStarboard",
	"TailDoorPort"]
const LEGS: Array = ["MainGearStarboard", "MainGearPort", "MainGearStarboardWheel", "MainGearPortWheel", "TailGear"]
const TYRES: Dictionary = {"MainGearStarboard": "MainGearStarboardWheel", "MainGearPort": "MainGearPortWheel",
	"TailGear": "TailGear"}
...
func _check(label: String, ok: bool, detail: String) -> void:
	print("[p47] %s %s (%s)" % ["PASS" if ok else "FAIL", label, detail])
	if not ok:
		_failures.append(label)


func _ready() -> void:
	var frame := P47Airframe.new()
	add_child(frame)
	frame.dress()
	_probe(frame)
	...
	_the_kinds_shape_is_the_drawn_draft(frame)
	...
	_it_has_the_jugs_belly_canopy_fin_and_eight_guns(frame)
	...
	_the_mains_shorten_as_they_fold(frame)
...
## THE KIND'S SHAPE IS THE AIRFRAME'S DRAFT, to the millimetre. `p47_shape` in cockpit_world.cpp was TYPED from this
## class's constants -- the box, the span and the mass -- and nothing but this holds the two together, so a constant
## changed in the drawing and not in the simulation is a hull the wrong size round a craft that still looks right.
## It also fixes the ORIGIN, which every craft-local number in the simulation is measured from: the eye, the seat, the
## eight muzzles and the keel's three stations.
func _the_kinds_shape_is_the_drawn_draft(frame: P47Airframe) -> void:
	var draft: Dictionary = frame.draft()
	var shape: Dictionary = Sim.geometry_of(Sim.Kind.P47)
	var drawn: Vector3 = draft["extents"]
	var typed: Vector3 = shape["extents"]
	_check("the_kinds_box_is_the_drawn_draft",
		(drawn - typed).length() < 0.001 and absf(float(draft["span"]) - float(shape["span"]) * 2.0) < 0.001,
		"drawn %.4v span %.4f, the kind's %.4v span %.4f" % [drawn, draft["span"], typed, float(shape["span"]) * 2.0])
	_check("the_kinds_mass_is_the_stall_tables_weight",
		absf(float(shape["mass"]) - 6001.0) < 1.0,
		"%.0f kg against the 13,230 lb Eng-47-1774-A's stall table is printed at" % [shape["mass"]])
...
func _probe(frame: Node3D) -> void:
	for mesh in _meshes(frame):
		...
		print("[p47] probe %-20s x %6.2f..%6.2f  y %6.2f..%6.2f  z %6.2f..%6.2f" % [mesh.name, box.position.x,
...
## surface at its stop.
func _it_is_one_object_and_not_a_set_of_parts(frame: P47Airframe) -> void:
...
## length and 0.3 of the printed span: [AN] DRAWS its span 0.95 per cent short of its own figure, and a wing built to the
## drawing would pass a 1 per cent check).
func _the_drawn_aeroplane_is_the_published_length_and_span(frame: P47Airframe) -> void:
	...
	_check("the_drawn_aeroplane_is_the_published_length_and_span",
		absf(length - PUBLISHED_LENGTH) <= PUBLISHED_LENGTH * 0.02 and absf(span - PUBLISHED_SPAN) <= PUBLISHED_SPAN * 0.02
			and absf(length - PRINTED_LENGTH) <= PRINTED_LENGTH * 0.01 and absf(span - PRINTED_SPAN) <= PRINTED_SPAN * 0.003,
...
func _the_tyres_stand_on_the_ground_level_and_three_point(frame: P47Airframe) -> void:
	frame.set_gear(1.0)
	var all := _box_of(frame, _meshes(frame))
	var mains: Array = []
	for leg in ["MainGearStarboardWheel", "MainGearPortWheel"]:
...
## THE HEIGHTS: level on the mains the fin's top stands the printed 13 ft 8-3/16 in over the ground (1 per cent); and tail
## down with a blade straight up, over the three-point ground, the published 4.47 m (2 per cent). Wikipedia gives no
## attitude for its figure; the first run put the model 4.491 m tall tail down against 4.171 level, so that is its
## attitude, and it is the THIRD QUANTITY: nothing the model was built to was either.
func _it_stands_the_published_height_tail_down(frame: P47Airframe) -> void:
	frame.set_gear(1.0)
	frame.set_props(0.0)
	var level: float = _box_of(frame, [frame.find_child("Fin", true, false)]).end.y + (frame.geometry()["extents"] as Vector3).y
	var main: AABB = _dark_box(frame, frame.find_child("MainGearStarboardWheel", true, false) as MeshInstance3D)
	var tail: AABB = _dark_box(frame, frame.find_child("TailGear", true, false) as MeshInstance3D)
	...
	_check("it_stands_the_printed_height_level_and_the_published_tail_down",
		absf(level - PRINTED_FIN_OVER_GROUND) <= PRINTED_FIN_OVER_GROUND * 0.01 and absf(highest - PUBLISHED_HEIGHT) <= PUBLISHED_HEIGHT * 0.02,
		"the fin's top %.3f m over the ground level on the mains (printed %.3f); tail down with a blade vertical %.3f m, raked %.2f deg (published %.2f)"
			% [level, PRINTED_FIN_OVER_GROUND, highest, rad_to_deg(rake), PUBLISHED_HEIGHT])
...
func _it_parks_tail_down_on_three_points(frame: P47Airframe) -> void:
	frame.set_gear(1.0)
	frame.set_props(0.0)
	...
	var said: PackedStringArray = []
	var ok: bool = true
	for leg in ["MainGearStarboardWheel", "MainGearPortWheel", "TailGear"]:
...
## THE ELLIPTICAL WING, from its drawn vertices: its leading and trailing edges at four stations of span against the
## plan's (both wings averaged, at the printed span), 2 cm; the middle surface's rise against the front view's; and the
## planform, integrated between the drawn edges with the chord at the fuselage's side carried to the centreline, against
## the drawn 26.94 m2 at the printed span (2 per cent).
const EDGES: Array = [[1.716, 2.707, 5.350], [3.231, 2.768, 5.143], [4.745, 2.871, 4.789], [5.856, 3.009, 4.186]]


func _the_wing_has_the_measured_edges_dihedral_and_area(frame: P47Airframe) -> void:
	...
	for p in _points(frame, frame.find_child("WingStarboard", true, false) as MeshInstance3D):
		...
		var drawn: float = p.x / P47Airframe.WING_STRETCH
		var hinged: bool = (drawn > P47Airframe.FLAP_SPAN.x + 0.01 and drawn < P47Airframe.FLAP_SPAN.y - 0.01) or \
			(drawn > P47Airframe.AILERON_SPAN.x + 0.01 and drawn < P47Airframe.AILERON_SPAN.y - 0.01)
		if not hinged:
			back[band] = maxf(float(back.get(band, -INF)), p.z)
		var b: Array = bands.get(band, [INF, -INF])
		bands[band] = [minf(b[0], p.y), maxf(b[1], p.y)]
	...
	for band in bands:
		...
		if x >= 2.5 and x <= 5.5:
			mid.append(Vector3(x, (float(bands[band][0]) + float(bands[band][1])) * 0.5, 0.0))
	...
	for row in EDGES:
		...
		said.append("%.2f out: %.3f-%.3f (%.3f-%.3f)" % [row[0], le, te, row[1], row[2]])
	var area: float = (_edge(back, 0.70) - _edge(front, 0.70)) * 0.70
	var x: float = 0.70
	var tip: float = P47Airframe.wing_tip()
	while x < tip:
		var x1: float = minf(x + 0.01, tip)
		area += ((_edge(back, x) - _edge(front, x)) + (_edge(back, x1) - _edge(front, x1))) * 0.5 * (x1 - x)
		x += 0.01
	area *= 2.0
	_check("the_wing_has_the_measured_edges_dihedral_and_area",
		worst < 0.02 and absf(dihedral - DIHEDRAL) < 0.5 and absf(area - DRAWN_WING_AREA) <= DRAWN_WING_AREA * 0.02,
		"edges %s, worst %.3f m; dihedral %.2f deg (%.2f); area %.2f m2 (drawn %.2f, published 27.87)"
			% [", ".join(said), worst, dihedral, DIHEDRAL, area, DRAWN_WING_AREA])


## THE THUNDERBOLT'S OWN SHAPES, from drawn vertices against the drawings': the DEEP BELLY that carries the turbocharger's
## ducting, 1.12 m under the thrust line under the wing, the fuselage 1.93 m deep there; the intercooler's exits on BOTH
## flanks; the bubble's top; the fin's top; the tailplane's span against the printed 16 ft 0-3/32 in; and EIGHT guns, four
## muzzles a wing at the front view's distances out, each ahead of its wing's drawn leading edge.
func _it_has_the_jugs_belly_canopy_fin_and_eight_guns(frame: P47Airframe) -> void:
	_rest(frame)
	...
	for p in _surface_points(frame, fuselage, 0):
		if p.z - nose > 2.95 and p.z - nose < 3.45:
			belly = minf(belly, p.y - thrust)
			deck = maxf(deck, p.y - thrust)
	for p in _surface_points(frame, fuselage, 1):
		glass_top = maxf(glass_top, p.y - thrust)
	var flanks := _points(frame, frame.find_child("Intercooler", true, false) as MeshInstance3D)
	var port: bool = false
	var starboard: bool = false
	for p in flanks:
		port = port or p.x < -0.5
		starboard = starboard or p.x > 0.5
	...
	for side in ["Starboard", "Port"]:
		...
		# EACH BARREL, its vertices grouped by the measured out they are nearest (barrels 0.15 m apart, 0.044 across).
		var groups: Dictionary = {}
		for p in _points(frame, guns):
			var key: int = 0
			for i in range(GUN_OUTS.size()):
				if absf(absf(p.x) - float(GUN_OUTS[i])) < absf(absf(p.x) - float(GUN_OUTS[key])):
					key = i
	...
	var guns_ok: bool = muzzles.size() == 8
	var said: PackedStringArray = []
	for m in muzzles:
		...
		said.append("%s %.2f out, %.2f ahead" % [m[0], m[1], le_there - float(m[2])])
	_check("it_has_the_jugs_belly_canopy_fin_and_eight_guns",
		absf(belly - BELLY) < 0.02 and deck - belly > 1.9 and port and starboard and glass_top > 1.1
			and absf(fin.end.y - thrust - FIN_TOP) < 0.02 and absf(tail.size.x - PRINTED_TAILPLANE) < PRINTED_TAILPLANE * 0.01
			and guns_ok,
		"belly %.3f under the wing (measured %.3f), the fuselage %.3f m deep there; intercooler exits port %s starboard %s; canopy top %.3f; fin top %.3f (%.3f); tailplane %.3f m (printed %.3f); %d muzzles: %s"
			% [belly, BELLY, deck - belly, port, starboard, glass_top, fin.end.y - thrust, FIN_TOP, tail.size.x,
				PRINTED_TAILPLANE, muzzles.size(), ", ".join(said)])
...
func _the_propeller_is_the_printed_size_and_turns_clockwise(frame: P47Airframe) -> void:
	frame.set_props(0.0)
	var mesh := frame.find_child("Propeller", true, false) as MeshInstance3D
	var hub: Vector3 = frame.point(0.0, 0.0, P47Airframe.PROP_STATION)
...
func _the_inner_doors_open_before_a_leg_moves_and_shut_after(frame: P47Airframe) -> void:
	frame.set_gear(1.0)
...
func _no_leg_ever_passes_through_a_door(frame: P47Airframe) -> void:
...
## gear down and within 1.3 m of the centreline, lying flat -- and every vertex of it over the underside the inner doors close, so it is
...
func _the_mains_fold_inward_and_the_tail_wheel_forward_inside_the_skin(frame: P47Airframe) -> void:
	frame.set_gear(1.0)
	var down: Dictionary = {}
	for leg in ["MainGearStarboardWheel", "MainGearPortWheel", "TailGear"]:
	...
	for leg in ["MainGearStarboardWheel", "MainGearPortWheel"]:
		...
		var inward: bool = absf(up.get_center().x) < absf((down[leg] as AABB).get_center().x) - 1.0 and absf(up.get_center().x) < 1.3
		...
		var wing := _solid(frame, "WingStarboard" if leg.contains("Starboard") else "WingPort")
		var poking: int = 0
		for i in range(pts.size()):
...
## THE MAINS SHORTEN AS THEY FOLD: each tyre's middle is TELESCOPE nearer the top of its leg's upper half -- its pivot
## end, the drawn strut's furthest point from the tyre -- with the gear up than down (1 cm).
func _the_mains_shorten_as_they_fold(frame: P47Airframe) -> void:
	var said: PackedStringArray = []
	var ok: bool = true
	for leg in ["MainGearStarboard", "MainGearPort"]:
		var reach := func() -> float:
			var tyre: Vector3 = _dark_box(frame, frame.find_child(leg + "Wheel", true, false) as MeshInstance3D).get_center()
			var far: float = 0.0
			var top := Vector3.ZERO
			for p in _points(frame, frame.find_child(leg, true, false) as MeshInstance3D):
				if p.distance_to(tyre) > far:
					far = p.distance_to(tyre)
					top = p
			return top.distance_to(tyre)
		frame.set_gear(1.0)
		var down: float = reach.call()
		frame.set_gear(0.0)
		var up: float = reach.call()
		ok = ok and absf((down - up) - TELESCOPE) < 0.01
		said.append("%s %.3f m down, %.3f up (%.3f shorter)" % [leg, down, up, down - up])
	_rest(frame)
	_check("the_mains_shorten_as_they_fold", ok, "; ".join(said) + " against the published 9 in, %.3f" % TELESCOPE)
...
func _the_hinged_surfaces_move_the_right_way_and_come_back(frame: P47Airframe) -> void:
	_rest(frame)
...
func _the_exterior_meets_the_first_lod_budget(frame: Node3D) -> void:
	...
	_check("the_exterior_meets_the_first_lod_budget", triangles <= 100000 and draws <= 40 and culled >= 3
		...
		"the whole P47Airframe subtree: %d triangles, %d draw surfaces, %d materials, %d distance-culled"
...
func _the_vat_draws_the_moving_parts_where_the_parts_are(parts: P47Airframe) -> void:
	var cast := P47Airframe.new()
	add_child(cast)
	cast.dress()
	...
	for a in [0.137, 0.613, 0.853]:
		...
		for part in ["MainGearStarboard", "MainGearStarboardWheel", "TailGear", "InnerDoorPort", "TailDoorStarboard",
				"AileronPort", "ElevatorPort", "Rudder", "FlapStarboard", "Propeller"]:
...
func _rest(frame: P47Airframe) -> void:
	frame.set_gear(1.0)
